...
def get_room(room_id, driver):
    ...
    driver.get(url)
    # 设置超时等待
    WebDriverWait(driver, 50, 0.5).until(lambda x:x.find_element_by_xpath('//div[@class="_1e7f1q6"]'))
...
class MyThread(threading.Thread):
    def __init__(self, room_id, executable_path, pathname):
    ...
    def run(self):
        ...
        driver = get_driver(self.executable_path)
        ...
        for j in self.room_id:
            ...
            try:
                # 获取房间信息
                room_picture_src, room_owner_id, room_owner_src, page = get_room(j, driver)

                # 获取评论信息
                reviews_id, reviews_rating, reviewers_id, reviewers_src = get_review(j, page)
...
if __name__ == '__main__':
    ...
    # 待爬取房间id
    # my_room_id = ['2044873', '14723039', '5590387']
    ...
    # 执行多线程
    t1 = MyThread(room_id=my_room_id_1, executable_path=my_executable_path, pathname=my_pathname)
    t2 = MyThread(room_id=my_room_id_2, executable_path=my_executable_path, pathname=my_pathname)
    t3 = MyThread(room_id=my_room_id_3, executable_path=my_executable_path, pathname=my_pathname)
